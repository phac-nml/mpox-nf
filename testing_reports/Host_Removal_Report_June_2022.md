...
Reads human-identified reads by kraken2 were run with command line blastn version 2.12.0+ on a snapshot of the NCBI nucleotide database from November 5th, 2020 to fully confirm that these reads were unlikely to have been of human origin. The top 10 lowest E-value scores for each read (both forward and reverse) were parsed with no human sequences identified. The top subject found was MPXV (Table 1). 
